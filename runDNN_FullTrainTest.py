...
from PreProcess import preDNN_processBatch
from loadFile_ import LoadbyFold, Hack4LoadbyFold
...
def runDNN_Train(feature_path, fold='config2_fold_1',useType=['Spec', 'Ceps', 'GCoS'], use_time_range = None):

    ############### Load and Preprocess Data ###############
    numVal = 30  #number of validation songs
    numTrain = 180  #number of training songs
    use_same_val = True  #wether to use the same validation set as last run
    ...
    data_shape = (1, tData_shape[2], tData_shape[3])
    print('\nInput shape: %s' % str(data_shape))

    ############### Define Model ###############
    initLR = 0.001
    num_middle_node = 512
    k_init = keras.initializers.RandomNormal(mean=0.0, stddev=0.05, seed=0)
    model_name = 'DNN_model_(' + fold + ')_' + str(useType) + '_' + str(use_time_range)
    ...
    for e in range(epoch):
        ...
        for i in range(train_batches):
            ...
            data, label = preDNN_processBatch(train_data, train_label, train_index, sel_t)
        ...
        for i in range(val_batches):
            ...
            data, label = preDNN_processBatch(val_data, val_label, val_index, sel_v)
    ...
    print('Best epoch: %d - Loss: %.4f - F-score: %.4f\n' % (best_epoch+1, (best_v/val_batches), (best_f/val_batches)))
    print('Execution time: %f sec' % (time.time()-start_t))          


    #Save training and validation history
    out_history = h5py.File("History/DNN_history("+fold+str(useType)+").hd5", 'w')
...
def runDNN_Test(model, feature_path, useType=['Spec', 'Ceps', 'GCoS'], fold='test_config2'):
    num_test = 60
    test_batchSize = 2000
    test_fold = fold+'.txt'
    ...
    for i in range(batches):
        ...
        data, label = preDNN_processBatch(Data, Label, Index, sel_t)
...
if __name__=='__main__':
    ...
    for i in range(4):
        ...
        use_specific_channel = [[0,1,2], [0,2], [2]]  #which feature type to use (0: spec, 1: ceps, 2: gcos)
        use_time_range = None
        #use_time_range = (0, 60)
        
        channel_type = ['Spec', 'Ceps', 'GCoS']
        for spec_ch in use_specific_channel:
            useType = [channel_type[i] for i in spec_ch]

            start_t = time.time()
            train_info, model = runDNN_Train(feature_path, fold, useType, use_time_range)
            exec_time = time.time()-start_t
            
            test_info = runDNN_Test(model, feature_path, useType, test_fold)

            print('Training result: \n' + train_info)
            print('Training time: %.4f sec' % exec_time)
            print('Testing result: \n' + test_info + '\n\n')
            log = open('DNN_log.txt','a')
